...
export class Renderer {
  ...
  draw(snap, cam, opts) {
    ...
    const soilFill = opts.soilFill || GROUND_FILL_DEFAULT;
    ...
    this._drawGround(ctx, opts.world, soilFill);
  ...
  _drawGround(ctx, world, fill = GROUND_FILL_DEFAULT) {
    if (!world) return;
    ctx.fillStyle = fill;
  ...
  // 搜寻者视野半径：外圈渐暗 (GDD 屏幕 60%)，边缘压暗强度约 30%
